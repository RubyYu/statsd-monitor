...
def disk():
    c = statsd.StatsClient(STATSD_SERVER, PORT, prefix=ip_address+'.system.disk')
    while True:
...
def cpu_times():
    c = statsd.StatsClient(STATSD_SERVER, PORT, prefix=ip_address+'.system.cpu')
    while True:
...
def memory():
    c = statsd.StatsClient(STATSD_SERVER, PORT, prefix=ip_address+'.system.memory')
    while True:
